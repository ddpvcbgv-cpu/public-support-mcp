    ...
    "tools": [
        {
            ...
            "description": """한국 공공 지원(복지, 주거, 의료, 고용 등) 탐색을 위한 통합 상담 도구.

호출 시점:
- 처음 상담을 시작할 때 (사용자가 상황을 설명하거나 지원을 요청할 때)
- 새로운 상황이나 추가 정보를 제공했을 때
- 전체적인 지원 탐색이 필요할 때

호출 금지:
- 사용자가 이미 특정 분야를 선택하고 "더 자세히", "구체적으로" 요청할 때 
  (이 경우 rank_support_cards 사용)
- 특정 분야의 카드만 추가로 보고 싶을 때 (rank_support_cards 사용)
- 이미 카드를 본 후 실행 방법만 물어볼 때 (generate_action_steps 사용)

기능: ①상황 요약 → ②분야 안내 → ③혜택 카드 2-3개 → ④행동 단계 → 
...
⚠️ 중요: 이미 orchestrate_full_response로 분야 안내를 받은 후, 
특정 분야를 선택하고 더 자세한 정보가 필요할 때 사용하세요.

핵심 분야: 주거·월세 | 생활 유지 | 의료·돌봄 | 고용·교육 | 심리·정서
확장 분야: 문화·여가 | 평생교육 | 참여·활동 (사용자 명시적 요청 시)

호출 시점:
- 사용자가 "주거 쪽으로 더 자세히", "의료 관련 지원이 궁금해요" 등 
  특정 분야를 명시적으로 선택했을 때
- "더 자세히", "구체적으로", "다른 옵션은?" 같은 후속 질문 시
- orchestrate_full_response로 분야 목록을 본 후 특정 분야 선택 시

호출 금지:
- 처음 상담 시작할 때 (orchestrate_full_response 사용)
- 분야가 불명확하거나 여러 개일 때 (orchestrate_full_response 사용)
- 전체적인 상황 분석이 필요할 때 (orchestrate_full_response 사용)
...
호출 금지:
- 사용자가 이미 특정 분야를 선택하고 "더 자세히", "구체적으로" 요청할 때
- 특정 분야의 카드만 추가로 보고 싶을 때
- 이미 카드를 본 후 실행 방법만 물어볼 때
