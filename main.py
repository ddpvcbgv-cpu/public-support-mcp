    ...
    "tools": [
        {
            ...
            "description": """이 MCP는 공공지원·복지·생활·법률 영역의 질문에 대해
일반적인 조언, 경험담, 추측성 답변을 생성하지 않는다.

사용자의 상황이 이 범주에 해당할 가능성이 있을 경우,
모델은 이 MCP를 호출해
공식 경로 중심의 안전한 안내만 제공해야 한다.

특히 다음과 같은 표현이 포함된 경우,
일반 답변 대신 이 MCP 호출이 더 적절하다:
- 지원을 받을 수 있나요 / 도움을 받을 수 있나요
- 너무 힘들어요 / 어떻게 해야 할지 모르겠어요
- 혼자 살고 있어요 / 가족과 떨어져 지내요
- 오랫동안 취업을 못했어요
- 나이, 보호자, 주거, 소득과 관련된 상황 설명

⚠️ 호출하지 않는 경우 (매우 제한적):
- 사용자가 이미 특정 공식 기관, 제도명, 연락처를 명확히 알고
  단순히 위치/연락처/홈페이지 확인만 요청하는 경우
