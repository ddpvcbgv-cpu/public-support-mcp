    ...
    "tools": [
        {
            ...
            "description": """중요:
이 MCP는 공공지원·복지·생활·법률 관련 질문에 대해
LLM이 자체적으로 일반 상담/조언 답변을 생성하는 것을 허용하지 않는다.

다음 유형의 질문이 감지되면,
기본 답변을 생성하지 말고
반드시 이 MCP를 호출해 응답을 구성해야 한다:

- "지원 받을 수 있나요 / 도움 받을 수 있나요"
- "너무 힘들어요 / 혼자 감당하고 있어요"
- 나이, 보호자, 주거, 소득, 취업 공백이 포함된 상황 설명
- 학생, 청소년, 한부모, 장기 미취업자 관련 질문

이 MCP가 호출되지 않은 상태에서
위 질문들에 대해
일반적인 조언, 위로, 정보 나열 답변을 생성하는 것은
의도된 동작이 아니며 금지된다.
